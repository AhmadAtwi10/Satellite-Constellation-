...
file_path = 'D:\Ahmad\France\Paris-Saclay\Courses\Semester 2\Joint Project\Joint Project\Constellations_ inforamtion\ProjectKuiperSecond.xlsx'
df = pd.read_excel(file_path)

# Calculate Cartesian coordinates for all satellites
df['Satellite-number'] = df['Satellite_number']
df['X'], df['Y'], df['Z'] = spherical_to_cartesian(df['Latitude'], df['Longitude'], 610 )
...
def find_neighbors(df):
    ...
    for orbit in range(36):  # 72 orbits numbered from 0 to 71
        orbit_data = df[(df['Satellite-number'] > orbit *36) & (df['Satellite-number'] <= (orbit + 1) * 36)]  # Satellites on the current orbit
...
LA_x, LA_y, LA_z = spherical_to_cartesian(LA_latitude, LA_longitude, 610)
...
source_node = 993  # Satellite 1 (Paris)
target_node = 712
...
for satellite_number in shortest_path:
    latitude = df.at[int(satellite_number) - 1, 'Latitude']  # Adjust for 0-based indexing
    longitude = df.at[int(satellite_number) - 1, 'Longitude']  # Adjust for 0-based indexing
...
# Coordinates for Satellite 712
lat_satellite1178 =  36.838215  # Latitude in degrees
lon_satellite1178 = -118.027084 # Longitude in degrees
alt_satellite1178 = 610        # Altitude in km
...
print("Round-Trip Time (RTT) between Satellite 712 and Los Angeles:", rtt_satellite1178_los_angeles, "seconds")
...
# Coordinates for Satellite 993
lat_satellite1 = 41.730744  # Latitude in degrees
lon_satellite1 = 3.534867   # Longitude in degrees
alt_satellite1 = 610       # Altitude in km
...
print("Round-Trip Time (RTT) between Paris and Satellite 993:", rtt_paris_satellite1, "seconds")
